...
source("r_data/eqtl_data.R")

for(i in 1:dim(to_pull[[variant_ix]])[1]) {
  
  if(to_pull[[variant_ix]]$source[i]=="api") {
    
    if(to_pull[[variant_ix]]$study[i]=="GTEx_V8") {
      eqtl_df = dplyr::filter(imported_tabix_paths, study==to_pull[[variant_ix]]$study[i], tissue_label==to_pull[[variant_ix]]$tissue[i])
    } else {
      eqtl_df = dplyr::filter(tabix_paths, study==to_pull[[variant_ix]]$study[i], tissue_label==to_pull[[variant_ix]]$tissue[i])
    }
    column_names = colnames(readr::read_tsv(eqtl_df$ftp_path[1], n_max=1))
    summary_stats = import_eqtl_catalog(eqtl_df$ftp_path[1], region_granges, selected_gene_id=to_pull[[variant_ix]]$ensembl_id[i], column_names)
    print(ggplot(summary_stats, aes(x=position, y=-log(pvalue,10))) + geom_point() + geom_vline(xintercept=hits$pos_38[variant_ix]) + theme_thesis(15) + ggtitle(paste(unlist(to_pull[[variant_ix]][i,]), collapse=", ")))
  ...
  if(to_pull[[variant_ix]]$source[i]=="eqtlgen") {
...
names(qtl_dat) = apply(to_pull[[variant_ix]], 1, function(x) paste(x[2], "Tissue:", x[4]))
...
source("r_data/traits_of_interest.R")
traits = traits_of_interest[[variant_ix]]
...
p5_with_legend = ggplot(coloc_plot_gwas, aes(x=start, y=p_value_scaled, color=Group)) + geom_point(alpha=0.5, size=1) + theme_thesis(15) + ylab("-log10(P) Scaled") + xlab("") + geom_vline(xintercept=hits$pos_38[variant_ix], alpha=0.5, color="grey", lty=2)
